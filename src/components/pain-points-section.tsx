...
const painPoints = [
    { icon: <XCircle className="text-destructive w-6 h-6 shrink-0 [text-shadow:0_0_8px_hsl(var(--destructive))]" />, text: "Sente que está ficando para trás na tecnologia?" },
    { icon: <XCircle className="text-destructive w-6 h-6 shrink-0 [text-shadow:0_0_8px_hsl(var(--destructive))]" />, text: "Já tentou de tudo para ganhar dinheiro online e falhou?" },
    { icon: <XCircle className="text-destructive w-6 h-6 shrink-0 [text-shadow:0_0_8px_hsl(var(--destructive))]" />, text: "Acha que IA é muito complicado para você?" },
]
...
export function PainPointsSection({ onComplete, onBack }: PainPointsSectionProps) {
  return (
    <div className="flex flex-col items-center justify-center text-center gap-8 max-w-2xl animate-fade-in">
        ...
        <Card className="bg-slate-900/30 backdrop-blur-md border border-destructive/20 p-6 md:p-8 rounded-2xl shadow-lg w-full text-left space-y-5 shadow-[0_0_20px_hsl(var(--destructive)/0.25)]">
            {painPoints.map((point, index) => (
                <div key={index} className="flex items-start gap-4 text-slate-300 text-lg">
                    {point.icon}
        ...
        <div className="flex flex-col items-center gap-4 animate-fade-in mt-4">
            <h3 className="text-2xl md:text-4xl font-bold text-white font-headline [text-shadow:0_0_15px_hsl(var(--primary)/0.3)]">
